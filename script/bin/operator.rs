...
use nuport_script::{relay, TendermintProver};
use log::{error, info};
...
const ELF: &[u8] = include_bytes!("../../elf/nuport-elf");
...
impl SP1NuportOperator {
    ...
    async fn check_vkey(&self) -> Result<()> {
        let contract = SP1Nuport::new(self.contract_address, self.wallet_filler.clone());
    ...
    async fn request_header_range(
        ...
        let inputs = prover
            .fetch_input_for_nuport_proof(trusted_block, target_block)
            .await;
    ...
    /// Relay a header range proof to the SP1 Nuport contract.
    ...
    async fn run(&mut self) -> Result<()> {
        info!("Starting SP1 Nuport operator");
...
async fn main() {
    ...
    let mut operator = SP1NuportOperator::new().await;
